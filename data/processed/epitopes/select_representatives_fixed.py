...
for idx, cluster in enumerate(clusters):
    ...
    for pep in cluster_peptides:
        ...
        # Handle N/A or float values
        cons_score = 0
        if cons != 'N/A' and not pd.isna(cons) and isinstance(cons, str) and '/' in cons:
            try:
                num, den = cons.split('/')
...
# Sort by Allele_Count descending
rep_df = rep_df.sort_values('Allele_Count', ascending=False)
