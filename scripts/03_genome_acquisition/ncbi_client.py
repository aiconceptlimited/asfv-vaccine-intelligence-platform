...
Functions:
- download_genbank_record: Download GenBank record
- download_fasta_record: Download FASTA record
- download_with_retry: Retry failed downloads
"""
...
def download_genbank_record(accession, email, retries=3, delay=1):
    """
    Download GenBank record for a given accession
    ...
    Raises:
        NCBIClientError: If download fails after all retries
    """
    Entrez.email = email
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                raise NCBIClientError(f"Failed to download {accession} after {retries} attempts: {e}")

def download_fasta_record(accession, email, retries=3, delay=1):
    """
    Download FASTA record for a given accession
    ...
    for attempt in range(retries):
        try:
            ...
            if record is None:
                raise NCBIClientError(f"Empty FASTA record returned for {accession}")
            ...
            if attempt < retries - 1:
                ...
                raise NCBIClientError(f"Failed to download FASTA for {accession} after {retries} attempts: {e}")
